...
class ImageEventRestorationModelEDGE(BaseModel):
    """Base Event-based deblur model for single image deblur."""

    def __init__(self, opt):
        super(ImageEventRestorationModelEDGE, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_perceptual:
            # print(self.opt['train']['perceptual_opt']['loss_weight'])   
            l_percep, l_style = self.cri_perceptual(self.output, self.gt, self.voxel)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            save_img_path = '/data2/yhe/NTIRE2025_EventDeblur_challenge-main/temp'
            if save_img:
                if self.opt['is_train']:
                    if cnt == 1: # visualize cnt=1 image every time
                        save_img_path = osp.join(
                        self.opt['path']['visualization'], f'{self.image_name}[0].png')
                        save_gt_img_path = osp.join(
                        self.opt['path']['visualization'], f'{self.image_name}[0]_gt.png')
                    save_img_path = osp.join(self.opt['path']['visualization'], f'{self.image_name}[0].png')
                    save_gt_img_path = osp.join(self.opt['path']['visualization'], f'{self.image_name}[0]_gt.png')
                else:
